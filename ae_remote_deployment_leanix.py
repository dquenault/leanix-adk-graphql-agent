...
dotenv.load_dotenv("./leanix_agent/.env")
...
from leanix_agent import agent
...
if update == "n":
    ...
    display_name="leanix_agent",description="Allows user to query our as-is application landscape through LeanIX", 
    agent_engine=agent.root_agent,
    requirements="requirements.txt",
    extra_packages=[
        "./leanix_agent", # a directory
    ],
    ...
    # Mostly for agent engine Console.
    display_name="leanix_agent",description=f"Allows user to query our as-is application landscape through LeanIX, updated {now.strftime("%Y_%m_%d_%H_%M_%S_%f")}", 
    agent_engine=agent.root_agent,
    requirements="requirements.txt",
    extra_packages=[
        "./leanix_agent", # a directory
